...
class backgroundSubtractionGMM:
    ...
    def __init__(self, number_of_gaussians, frame_read_per_second):
    ...
    def resetNonMatchingGaussian(self, row_index, col_index, pixel_intensity, likelihood, matched_gaussian):
        if np.sum(matched_gaussian) == 0:
            ...
            self.mean_array[row_index, col_index, argmin_gaussian] = 0.025*pixel_intensity
    ...
    def fitMixtureOfGaussians(self, MAX_ITERATIONS=200):
        ...
        for time_index in range(self.num_frames):
            ...
            for row_index in range(self.n_rows):
                for col_index in range(self.n_cols):
                    ...
                    curr_weights_array = (1-self.learning_rate)*curr_weights_array + self.learning_rate*matched_gaussians
                    ...
                    updated_mean_array[matched_gaussians]= ((1-learning_rate_rho)*curr_mean_array + learning_rate_rho*pixel_intensity)[matched_gaussians]
                    updated_variance_array[matched_gaussians] = ((1-learning_rate_rho)*curr_variance_array + learning_rate_rho * (pixel_intensity-curr_mean_array)**2)[matched_gaussians]
...
for i in range(a.num_frames):
    cv2.imwrite('background_'+str(i)+'.png', a.per_frame_background_array[:, :, i])
