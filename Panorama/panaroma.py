...
def testHomography(distance_threshold, random_index_list, source_features, target_features, test_homography_matrix):
    ...
    for index, source_target_feature_point in enumerate(zip(source_features, target_features)):
        if index in random_index_list:
            ...
            source_homogeneous_point = convertToHomogeneousCoord(np.array([source_target_feature_point[0]]))
    ...
    return count / total_test_points * 100 >= 90, count / total_test_points, valid_index_list
...
def siftFeatureMatchMatlab(source_image, target_image, k_size=3, blockSize=16, k=0.01):
...
def getHomographyMatrix(source_image, target_image, MAX_ITERATIONS=100):
    source_features, target_features = siftFeatureMatchMatlab(source_image, target_image)
    iterations = 0
    H_list = []
    accuracy_list = []
    while iterations <= MAX_ITERATIONS:
        ...
        is_valid_homography, accuracy, valid_index_list = testHomography(10, random_index_list, source_features, target_features,
...
cv2.imwrite('./output/canvas.png', canvas_img)
